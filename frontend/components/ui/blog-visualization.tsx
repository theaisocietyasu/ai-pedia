    ...
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="space-y-2">
          <h4 className="text-sm font-medium text-white text-center">Input Layer</h4>
          <div className="space-y-2">
            {[1, 2, 3].map(i => (
              <motion.div
                key={i}
        ...
        {/* hidden layer */}
        <div className="space-y-2">
          <h4 className="text-sm font-medium text-white text-center">Hidden Layer</h4>
          <div className="space-y-2">
            {[1, 2, 3, 4].map(i => (
              <motion.div
                key={i}
        ...
        {/* output layer */}
        <div className="space-y-2">
          <h4 className="text-sm font-medium text-white text-center">Output Layer</h4>
          <div className="space-y-2">
            {[1, 2].map(i => (
              <motion.div
                key={i}
                animate={{ scale: [1, 1.1, 1] }}
                transition={{ duration: 2, repeat: Infinity, delay: i * 0.4 + 2 }}
  ...
  TransformerVisualization: () => (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="space-y-4">
          <h4 className="text-sm font-medium text-white text-center">Input Embeddings</h4>
          <div className="space-y-2">
        ...
        <div className="space-y-4">
          <h4 className="text-sm font-medium text-white text-center">Self-Attention</h4>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            {["Le", "chat", "était", "assis", "sur", "le", "tapis"].map((word, i) => (
              <motion.div
                key={word}
                animate={{ opacity: [0.5, 1, 0.5] }}
                transition={{ duration: 2, repeat: Infinity, delay: i * 0.15 + 1 }}
  ...
  LinearRegressionDemo: () => (
    <div className="space-y-4">
      <div className="relative aspect-square bg-dark-gray rounded-lg border border-white/10 p-4">
        <svg width="100%" height="100%" viewBox="0 0 300 300" className="text-white">
          {/* grid lines */}
          <defs>
          ...
          {/* data points */}
          {[
            [50, 250], [80, 220], [120, 180], [160, 140], [200, 100], [240, 80]
          ].map(([x, y], i) => (
            <motion.circle
              key={i}
              cx={x}
...
model.fit(x_train, y_train, epochs=5)`}</code>
      ...
      <div className="flex items-center justify-center gap-4 p-4 bg-white/5 rounded-lg">
        ...
        />
        <span className="text-sm text-light-gray/80">Training in progress...</span>
      </div>
    </div>
  )
}
