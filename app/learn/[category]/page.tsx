...
export async function generateMetadata({ params }: CategoryPageProps): Promise<Metadata> {
  ...
  try {
    ...
    const description = categoryData.description ||
      `Explore ${modules.length} comprehensive tutorials and guides about ${category.replace(/-/g, ' ')}. Master key concepts with our interactive learning modules.`

    return {
      title: `${category.charAt(0).toUpperCase() + category.slice(1).replace(/-/g, ' ')} Learning`,
      description: description,
      keywords: keywords,
      openGraph: {
        title: `${category.charAt(0).toUpperCase() + category.slice(1).replace(/-/g, ' ')} Learning | AI Pedia`,
        description: description,
...
async function CategoryPage({ params }: CategoryPageProps) {
  ...
  const collectionSchema = {
    ...
    "@type": "CollectionPage",
    "name": `${category.charAt(0).toUpperCase() + category.slice(1).replace(/-/g, ' ')} Learning`,
    "description": categoryDescription || `Learning resources about ${category.replace(/-/g, ' ')}`,
    "url": `${baseUrl}/learn/${category}`,
    "provider": {
      "@type": "Organization",
