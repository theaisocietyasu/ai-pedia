  ...
  for (let i = 0; i < count; i++) {
    ...
    switch (type) {
      ...
      case "nonlinear":
        ...
        break;
      case "nonnormal":
        residual = Math.random() ** 2 * (Math.random() > 0.5 ? 4 : -4);
        break;
    }
...
export const calculateRSquared = (
  ...
  const ssTotal = actual.reduce((sum, val) => sum + (val - actualMean) ** 2, 0);
...
export const responsiveUtils = {
  ...
  getResponsiveLayout: (direction: "horizontal" | "vertical" = "horizontal") =>
    direction === "horizontal" ? "flex flex-col lg:flex-row" : "flex flex-col",
