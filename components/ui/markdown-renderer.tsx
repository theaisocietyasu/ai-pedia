...
export function MarkdownRenderer({
  ...
  const [parsedContent, setParsedContent] = useState<ParsedMarkdown | null>(
    null,
  );
  ...
  useEffect(() => {
    if (
      ...
      !showTableOfContents ||
      !parsedContent?.tableOfContents.length
    )
      return;
    ...
    const handleScroll = () => {
      ...
      const headings = parsedContent.tableOfContents
        .map((item) => ({
          id: item.id,
      ...
      const crossedHeadings = headingsWithPosition.filter(
        (h) => h.top <= HEADER_OFFSET,
      );
  ...
  const renderWithVisualizations = (html: string) => {
    ...
    const parts = html.split(
      /(<div class="markdown-visualization" data-component-id="[^"]+"><\/div>)/g,
    );
    ...
    return parts.map((part, index) => {
      ...
      if (match) {
        ...
        const Component =
          visualizationComponents[componentId] ||
    ...
    >
      ...
      >
        <span
          ...
          ${activeHeadingId === item.id ? "bg-purple" : "bg-light-gray/30"}
        `}
        />
            ...
            >
              <nav>
                <ul className="space-y-1">
                  {MarkdownParser.buildHierarchicalToc(
